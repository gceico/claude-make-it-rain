...
} from '../src/lib/leaderboard-client';

const tmpRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'mir-lb-test-'));
const configFile = (dir: string) => path.join(dir, 'config.json');

// ── Tag generation ───────────────────────────────────────────────────────────
test('tag generation (shape + variety)', () => {
  ...
  assert.ok(
    seen.size > 100,
    `tags should vary widely, got ${seen.size} distinct`
  );
});
...
test('normalizeConfig', () => {
  ...
  assert.ok(
    fresh.gamerTag && typeof fresh.gamerTag === 'string',
    'generates a tag'
  );
  ...
  assert.strictEqual(
    custom.reportIntervalMs,
    120000,
    'respects custom interval'
  );
...
test('legacy apiBaseUrl migration', () => {
  assert.strictEqual(
    normalizeConfig({ apiBaseUrl: 'https://make-it-rain.example.com' })
      .apiBaseUrl,
    DEFAULT_API_BASE_URL,
  ...
  assert.strictEqual(
    normalizeConfig({
      apiBaseUrl: 'https://claude-make-it-rain-production.up.railway.app',
    }).apiBaseUrl,
    DEFAULT_API_BASE_URL,
    'the retired Railway reference URL migrates to the current default'
  );
  assert.strictEqual(
    normalizeConfig({ apiBaseUrl: 'https://my-own-leaderboard.example.org' })
      .apiBaseUrl,
...
test('security: apiBaseUrl scheme allow-list + tag re-sanitization on load', () => {
  ...
  for (const bad of [
    ...
    assert.strictEqual(
      isSafeHttpUrl(bad),
      false,
      `isSafeHttpUrl rejects ${JSON.stringify(bad)}`
    );
    assert.strictEqual(
  ...
  assert.strictEqual(
    ...
    'HTML gamerTag sanitized on load'
  );
  assert.ok(
    !/[<>"'&]/.test(evil.gamerTag),
    'no HTML-significant chars survive load'
  );
  ...
  assert.strictEqual(
    loaded.apiBaseUrl,
    DEFAULT_API_BASE_URL,
    'file: URL neutralized on load'
  );
  assert.strictEqual(
    loaded.gamerTag,
    'scriptxscript',
    'HTML tag neutralized on load'
  );
});

// ── First-run creation + persistence round-trip ──────────────────────────────
test('config persistence (first-run, stability, recovery, round-trip)', () => {
...
test('client toggle + tag management', () => {
  ...
  assert.strictEqual(
    loadConfig(dir).telemetryEnabled,
    false,
    'toggle persisted'
  );
  ...
  assert.ok(
    typeof newTag === 'string' && newTag.length > 0,
    'reroll returns a tag'
  );
...
// lib/config.ts (mute preference) and this client persist to the SAME file.
...
test('unified config store (cross-feature keys survive round-trips)', async () => {
  ...
  assert.strictEqual(
    onDisk.muted,
    true,
    'leaderboard save must preserve muted'
  );
  assert.strictEqual(
    onDisk.gamerTag,
    'UnifiedYak1234',
    'leaderboard save wrote the tag'
  );
  ...
  assert.strictEqual(
    onDisk.gamerTag,
    'UnifiedYak1234',
    'mute toggle must preserve gamer tag'
  );
  assert.strictEqual(onDisk.muted, false, 'mute toggle persisted');
  assert.strictEqual(
    onDisk.telemetryEnabled,
    true,
    'telemetry key survives too'
  );
  ...
  assert.strictEqual(
    new Config(configFile(dir)).get('muted'),
    false,
    'Config sees mute'
  );
});
...
test('client reporting (payload minimalism, disable, fail-silent)', async () => {
  const dir = path.join(tmpRoot, 'report');

  // Enabled: sends one well-formed POST.
  let captured: { url: string; opts: { body: string } } | null = null;
  const okFetch = async (url: string, opts: unknown) => {
    captured = { url, opts: opts as { body: string } };
    return { ok: true, status: 200 };
  };
  const c1 = new LeaderboardClient({
    configDir: dir,
    getTotal: () => 12.345,
    fetchImpl: okFetch,
  });
  ...
  assert.deepStrictEqual(
    ...
    ['tag', 'total'],
    'body ONLY has tag+total (no PII)'
  );
