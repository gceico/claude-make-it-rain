 ...
 * of lib/leaderboard-client.js: one attempt per tick, short AbortController
...
const REGISTRY_URL = 'https://registry.npmjs.org/@gceico/claude-make-it-rain/latest';
 ...
 */
function shouldNotify(version, lastNotifiedVersion) {
...
class UpdateChecker {
  /**
   * @param {object} opts
   * @param {string} opts.currentVersion  the running app's version
   * @param {number} [opts.checkIntervalMs]  poll interval (default 24h)
   * @param {typeof fetch} [opts.fetchImpl]  injectable for tests
   * @param {(info: {version: string}) => void} [opts.onUpdateAvailable]  fired when a newer version is seen
   * @param {string} [opts.registryUrl]  override for tests
   */
  constructor({ currentVersion, checkIntervalMs, fetchImpl, onUpdateAvailable, registryUrl } = {}) {
    this.currentVersion = typeof currentVersion === 'string' ? currentVersion : '0.0.0';
    this.checkIntervalMs =
      typeof checkIntervalMs === 'number' && isFinite(checkIntervalMs) && checkIntervalMs >= 60000
    ...
    try {
      ...
      if (latest && isNewerVersion(latest, this.currentVersion)) {
        ...
        if (this.onUpdateAvailable) {
          try { this.onUpdateAvailable(info); } catch { /* callback must not break the check */ }
        }
        return info;
      }
      return null;
