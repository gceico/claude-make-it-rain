...
function parseUsageEntry(line) {
  ...
  // Dedup key: requestId:messageId. A missing message.id yields a random uuid
  // so entries lacking an id are never collapsed into one another.
  const requestId = typeof obj.requestId === 'string' ? obj.requestId : '';
  const msgId = typeof message.id === 'string' ? message.id : crypto.randomUUID();
